...
export default function NotificationBell() {
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-2xl z-[999] overflow-hidden">
          {/* Header */}
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <button
                  onClick={markAllRead}
                  disabled={loading}
                  className="flex items-center gap-1 text-xs text-makati-blue dark:text-blue-400 hover:underline disabled:opacity-50"
                >
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 flex gap-3 transition-colors ${
                      n.isRead ? "opacity-50 cursor-default" : "hover:bg-gray-50 dark:hover:bg-slate-800 cursor-pointer"
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {!n.isRead && (
                          <span className="w-2 h-2 rounded-full bg-makati-blue dark:bg-blue-500 shrink-0 mt-1.5" />
                        )}
                      </div>
                      <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5 line-clamp-2">{n.message}</p>
                      <p className="text-[10px] text-gray-400 dark:text-slate-600 mt-1">{timeAgo(n.createdAt)}</p>
                    </div>
                  </button>
                );
              })
